...
// Add a function to get trending articles
export const getTrendingArticles = (count = 5) => {
    // Generate trending articles with breaking news headlines
    const trendingArticles = [];
    
    // Trending headline templates
    const trendingHeadlines = [
        "BREAKING: {company} Announces {announcement} That Shocks {sector} Industry",
        "TRENDING: {company} Stock {movement} {percent}% After Surprise {event}",
        "JUST IN: {company} and {company2} Announce Game-Changing Partnership",
    ...
    for (let i = 0; i < count; i++) {
        // Pick random companies and sector
        const company = companies[Math.floor(Math.random() * companies.length)];
        let company2;
        do {
            company2 = companies[Math.floor(Math.random() * companies.length)];
        } while (company2 === company);
        
        const sector = sectors[Math.floor(Math.random() * sectors.length)];
        const source = sources[Math.floor(Math.random() * sources.length)];
        
        // Generate headline
        const headlineTemplate = trendingHeadlines[Math.floor(Math.random() * trendingHeadlines.length)];
        const movement = Math.random() > 0.5 ? "Surges" : "Plunges";
        const percent = Math.floor(Math.random() * 15) + 2; // 2-15%
        const announcement = ["Major Acquisition", "Strategic Partnership", "New Product Line", "Restructuring Plan", "Earnings Beat"][Math.floor(Math.random() * 5)];
        const event = ["Earnings Report", "CEO Departure", "Product Launch", "Regulatory Decision", "Market Shift"][Math.floor(Math.random() * 5)];
        const challenge = ["Supply Chain Issues", "Regulatory Scrutiny", "Competitive Pressure", "Product Recall", "Labor Dispute"][Math.floor(Math.random() * 5)];
        ...
        trendingArticles.push({
            _id: `trending-${i}-${Date.now()}`,
            _source: {
                headline,
                url: `http://example.com/trending/${company.toLowerCase().replace(/\s+/g, '-')}`,
                published_at: publishedDate.toISOString(),
                source,
                snippet: `Breaking news about ${company} in the ${sector} sector. This trending story is developing...`,
                ...
                companies: [
                    ...
                    {
                        name: company2,
                        ticker: company2.substring(0, 3).toUpperCase(),
                        is_primary: false,
                ...
                relevance_score: (Math.random() * 0.2) + 0.8 // 0.8-1.0 for trending
            }
        });
    }
